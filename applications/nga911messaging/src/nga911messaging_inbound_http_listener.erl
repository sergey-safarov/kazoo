...
%%% @end
%%%-----------------------------------------------------------------------------
-module(nga911messaging_inbound_http_listener).
...
-define(LISTENING_PORT, 32517).

-spec start_link() -> kz_types:startlink_ret().
start_link() ->
    kz_util:put_callid(?DEFAULT_LOG_SYSTEM_ID),

    Dispatch = cowboy_router:compile([{'_', [{<<"/telnyx/[...]">>, [], 'nga911messaging_inbound_http_handler', []}
                                            ]}
                                     ]),
...
maybe_start_plaintext(Dispatch) ->
    case kapps_config:get_is_true(?CONFIG_CAT, <<"use_plaintext">>, 'true') of
        ...
        'true' ->
            Port = kapps_config:get_integer(?CONFIG_CAT, <<"proxy_port">>, ?LISTENING_PORT),
            IP = get_binding_ip(),
