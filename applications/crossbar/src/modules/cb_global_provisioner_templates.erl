...
-spec content_types_provided(cb_context:context(), http_method(), path_token(), path_token()) -> crossbar_content_handlers().
...
content_types_accepted(Context, ?HTTP_POST, _, ?IMAGE_REQ) ->
    cb_context:set_content_types_accepted(Context, [{'from_binary', ?MIME_TYPES}]);
content_types_accepted(Context, _, _, ?IMAGE_REQ) ->
    Context.
